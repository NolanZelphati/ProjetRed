...
func (C *character) Init(name string, class string, lvl int, maxlife int, life int, manamax int, mana int, init int, money int, slotinv int, inventory []string, skill []string, head string, body string, foot string) {
	C.name = name
	C.class = class
	C.lvl = lvl
	C.maxlife = maxlife
...
func (M *Monstre) Init(name string, pvmax int, pv int, dps int, init int) {
	M.name = name
	M.pvmax = pvmax
	M.pv = pv
	M.dps = dps
...
func main() {
	C1.Init(C.name, C.class, 1, C.maxlife, 1, 50, 50, 10, 100, 10, []string{"potion de santé", "potion de santé", "potion de santé"}, []string{"coup de poing"}, "empty", "empty", "empty")
	M1.Init("Gobelin d'entrainement", 40, 40, 5, 5)
	C1.charcreation()
	for {
		if C1.life <= 0 {
			C1.dead()
...
func (C *character) PotiondeMana() {
	...
	if C.mana >= C.manamax-25 {
		fmt.Println("Vous utilisez une potion de soin, vous regagnez 25 pts de mana")
		C.mana = C.manamax
		C.removestuff(i)
	} else {
		fmt.Println("Vous utilisez une potion de soin, vous regagnez 25 pts de mana")
...
// Return value to know if we need to go back to the menu
func (C *character) charTurn() bool {
	...
	case 2:
		C.accesinventory()
	case 3:
		fmt.Println("Vous prenez la fuite telle un miserable gueux apeuré.")
		time.Sleep(2 * time.Second)
		return true
	}
...
func (C *character) verfsort(rang int, mana int) {
	if rang >= len(C.skill) {
		return
	}
	if C.skill[rang] == "coup de poing" {
		...
		if M1.pv-8 < 0 {
			...
			fmt.Println("------------------------------------------------")
			time.Sleep(1 * time.Second)
			fmt.Println(C1.name, "inflige à", M1.name, 8, "pts de dégat", "votre Mana restant est de", C.mana, "/", C.manamax)
			time.Sleep(1 * time.Second)
	...
	if C.skill[rang] == "boule de feu" {
		...
		if M1.pv-18 < 0 {
			...
			fmt.Println("------------------------------------------------")
			time.Sleep(1 * time.Second)
			fmt.Println(C1.name, "inflige à", M1.name, 18, "pts de dégat", "votre Mana restant est de", C.mana, "/", C.manamax)
			time.Sleep(1 * time.Second)
			fmt.Println("il reste", M1.pv, "/", M1.pvmax, "pts de vie à", M1.name)
			time.Sleep(1 * time.Second)
			fmt.Println("------------------------------------------------")
